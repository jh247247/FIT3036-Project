...
{\bf TODO:} figure of the program update flow?

\subsection{World}
The world is implemented in a two dimensional grid, with opposing
sides connected to one another. At each time step the following is
calculated:
\begin{itemize}
\item The average temperature
\item The average albedo
\item The expected temperature from the average albedo
  (TODO Stefan)
\item The expected temperature for a lifeless world
\item The amount of each type of daisy
\end{itemize}

These numbers are written to a file for later processing

Then, the following actions are completed:
\begin{itemize}
\item Allowing each tile to update it's attributes
\item Temperature mixing between adjacent tiles
\end{itemize}
